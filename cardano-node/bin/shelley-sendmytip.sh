...
if [[ -z "${CARDANO_POOL_ID}" ]]; then
  echo "Required CARDANO_POOL_ID env var is not set. Exiting..."
  exit 1
fi

if [[ -z "${POOL_TOOL_API_KEY}" ]]; then
  echo "Required POOL_TOOL_API_KEY env var is not set. Exiting..."
  exit 1
fi
...
while true ; do
  ...
  if [ "${lastBlockHeight}" != "" ] && [ "${lastBlockHeight}" -gt "${lastBlockHeightSeen}" ]; then
    echo "New height"
    ...
    RESPONSE="$(curl -s -H "Accept: application/json" -H "Content-Type:application/json" -X POST --data "$JSON" "https://api.pooltool.io/v0/sendstats" || true)"
    echo "${RESPONSE}"
    lastBlockHeightSeen="${lastBlockHeight}"
  fi
done