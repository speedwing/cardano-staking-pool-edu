...
if [[ -z "${CNODE_HOSTNAME}" ]]; then
  echo "Required CNODE_HOSTNAME env var is not set. Exiting..."
  exit 1
fi

if [[ -z "${CNODE_TOPOLOGY}" ]]; then
  echo "Required CNODE_TOPOLOGY env var is not set. Exiting..."
  exit 1
fi
...
CNODE_LOG_DIR="/var/log/"                       # Folder where your logs will be sent to (must pre-exist)
